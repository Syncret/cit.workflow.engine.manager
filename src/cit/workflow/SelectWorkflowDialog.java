	...
	private class PackageListListener implements ListSelectionListener {
		public void valueChanged(ListSelectionEvent event) {
			...
			try {
				...
				String[] columns = { "WorkflowID", "WorkflowName",
						"Description" };
				String selectedPackageName = (String) packageNames
						.get(packageIndex);
				data = getWorkflowColumnsbyPackageID(selectedPackageName,
						columns);
				// logger.info("选中包 - " + selectedPackageName);
				WorkflowListTableModel wfModel = new WorkflowListTableModel(
						data);
				...
				WFmodelTable.addMouseListener(new MouseListener() {
					public void mouseClicked(MouseEvent e) {
						...
						// zy 2008.05.16 add
						try {
							workflowID = Integer.parseInt(WFmodelTable
		...
		public Object[][] getWorkflowColumnsbyPackageID(String packageName,
				String[] columns) {
			String connstr = "";
			...
			try {
				...
				connstr += " from WorkflowInformation,PackageInformation where PackageInformation.PackageID=WorkflowInformation.PackageID and PackageName='"
						+ packageName
						+ "' order by WorkflowInformation.WorkflowID asc";
